...
- [ ] **大画布重设计** — 整个页面就是办公室场景（全屏画布），移除当前的分区卡片布局
- [ ] **Agent 旁对话框** — 每个 Agent 旁边显示聊天气泡/对话框，位置根据 Agent 位置自动判断左右
- [ ] **聊天分配** — 对话框直接对接该 Agent 的聊天 API（`/api/chat/stream?group={agentId}`），每个 Agent 独立会话
