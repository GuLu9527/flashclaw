 ...
 * @param chatJid 聊天 ID
 * @param limit 最大消息数量
 * @param beforeTimestamp 可选，获取此时间之前的消息
 */
export function getChatHistory(chatJid: string, limit = 50, beforeTimestamp?: string): NewMessage[] {
  ...
  return db.prepare(`
    ...
    ORDER BY timestamp DESC
    LIMIT ?
  `).all(chatJid, limit).reverse() as NewMessage[];
}
...
export function getMessageStats(chatJid: string): { totalMessages: number; firstMessage: string | null; lastMessage: string | null } {
  const stats = db.prepare(`
    ...
    FROM messages
    WHERE chat_jid = ?
  `).get(chatJid) as { total: number; first_msg: string | null; last_msg: string | null };
  
  return {
    totalMessages: stats.total,
    firstMessage: stats.first_msg,
    lastMessage: stats.last_msg
  };
}
